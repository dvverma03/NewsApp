...
export default function UnitedStateScreen() {
  ...
  useEffect(() => {
      const fetchData = async () => {
          try {
              const response = await fetch(`https://newsapi.org/v2/top-headlines?country=us&category=${category}&apiKey=354192c227f0409782a687e6f8e99b8b`);
              if (!response.ok) {
  ...
  if(Articles===null) return <LoadingOverlay/> 
  ...
  return (
    <GestureHandlerRootView>
      ...
      <View style={styles.Box}>
        {Articles &&
          Articles.map((e) => (
            <Pressable key={e.title} onPress={()=> navigation.navigate("Details News", {article:e})}>
